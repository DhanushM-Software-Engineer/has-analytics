...
function node(attributes, node_id = 1) {
  const data = {
    node_id,
    date_commissioned: "",
    last_interview: "",
...
function binding(p) {
  return { node: 2, group: void 0, endpoint: 1, cluster: 6, fabricIndex: 1, ...p };
}
describe("binding util", () => {
  ...
  it("treats null binding fields as unset, not 0", () => {
    ...
    expect(b.cluster).to.equal(void 0);
  });
  ...
  it("reverseAclState returns present/missing/overPrivileged/cannotVerify", () => {
    ...
    expect(reverseAclState(1, binding({ cluster: 8 }), void 0).state).to.equal("cannotVerify");
