...
export class DashboardHeader extends LitElement {
    ...
    // When embedded (analytics dashboard opens the UI with ?ac=1), hide the whole
    // header/nav bar — the host page provides its own navigation.
    private readonly _embedded = new URLSearchParams(location.search).get("ac") === "1";
    ...
    protected override render() {
        // Hidden entirely in embedded mode
        if (this._embedded) return nothing;
        return html`
            <div class="header">
